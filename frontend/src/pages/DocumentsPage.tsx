    ...
    <div className="space-y-6 relative" {...dropHandlers}>
      ...
      {dragging && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-primary/10 backdrop-blur-sm pointer-events-none">
          <div className="bg-card rounded-2xl shadow-2xl p-10 flex flex-col items-center gap-3 border-2 border-dashed border-primary">
            ...
            <p className="text-sm text-muted-foreground">PDF, JPG, PNG, HTML supported</p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground font-medium">Manage and review extracted receipt data</p>
        </div>
      ...
      {/* ── Filter bar ──────────────────────────────────────────────── */}
      <div className="bg-card rounded-xl shadow-[0_2px_8px_rgba(25,28,30,0.04)] p-4">
      ...
      <div className="flex items-center justify-between">
        <span className="text-sm text-muted-foreground font-medium">{total} document{total !== 1 ? "s" : ""}</span>
        <div className="flex gap-2 items-center">
          <button
            disabled={page <= 1}
            onClick={() => setPage(page - 1)}
            className="p-2 rounded-lg hover:bg-card disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
