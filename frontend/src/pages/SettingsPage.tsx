        ...
        <TabsContent value="llm" className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div><Label>Temperature (0 = deterministic, higher = more creative)</Label><Input type="number" step="0.1" min="0" max="2" value={settings.llm_temperature ?? 0} onBlur={(e) => save({ llm_temperature: parseFloat(e.target.value) })} onChange={(e) => setSettings({ ...settings, llm_temperature: e.target.value })} /></div>
              <div><Label>Max Output Tokens</Label><Input type="number" step="1024" min="1024" max="32768" value={settings.llm_max_tokens ?? 8192} onBlur={(e) => save({ llm_max_tokens: parseInt(e.target.value) || 8192 })} onChange={(e) => setSettings({ ...settings, llm_max_tokens: e.target.value })} /></div>
              <div><Label>Sleep Between Calls (seconds)</Label><Input type="number" step="0.1" value={settings.llm_sleep_interval ?? ""} onBlur={(e) => save({ llm_sleep_interval: parseFloat(e.target.value) })} onChange={(e) => setSettings({ ...settings, llm_sleep_interval: e.target.value })} /></div>
              <div><Label>Confidence Threshold (0 = never flag for review)</Label><Input type="number" step="0.05" min="0" max="1" value={settings.confidence_threshold ?? ""} onBlur={(e) => save({ confidence_threshold: parseFloat(e.target.value) })} onChange={(e) => setSettings({ ...settings, confidence_threshold: e.target.value })} /></div>
