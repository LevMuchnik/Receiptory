...
function UploadDialog({ onClose, onUpload }: UploadDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-card rounded-xl shadow-2xl p-6 w-96 max-w-full mx-4" onClick={(e) => e.stopPropagation()} {...dropHandlers}>
        ...
        <label className={`flex flex-col items-center justify-center border-2 border-dashed rounded-xl p-8 cursor-pointer transition-all group ${
          dragging ? "border-primary bg-primary/5 scale-[1.02]" : "border-border hover:border-primary hover:bg-card"
        }`}>
          <span className={`material-symbols-outlined text-4xl mb-2 ${dragging ? "text-primary" : "text-muted-foreground group-hover:text-primary"}`}>
            {dragging ? "file_download" : "upload_file"}
          </span>
          <span className="text-sm font-semibold text-foreground">{dragging ? "Drop files here" : "Drag & drop or click to upload"}</span>
          <span className="text-xs text-muted-foreground mt-1">PDF, JPG, PNG, HTML supported</span>
...
export default function Sidebar({ mobileOpen, onMobileClose }: SidebarProps) {
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-4 py-3 rounded-lg font-headline font-semibold tracking-tight transition-colors duration-200 ${
                isActive
                  ? "text-primary bg-background font-bold"
      ...
      {/* User info */}
      {username && (
        <div className="mb-3 px-2 py-2 rounded-lg bg-background flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="material-symbols-outlined text-muted-foreground text-lg">account_circle</span>
            <span className="text-xs font-semibold text-foreground truncate max-w-[100px]">{username}</span>
          </div>
          <button
            onClick={logout}
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="fixed left-0 top-0 h-full w-64 hidden md:flex flex-col p-4 bg-card z-50 border-r-0">
        {sidebarContent}
      </aside>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <aside className="absolute left-0 top-0 h-full w-64 flex flex-col p-4 bg-card shadow-2xl">
            {sidebarContent}
          </aside>
        </div>
      )}
      ...
      {/* Mobile bottom nav */}
      <nav className="fixed bottom-0 left-0 w-full h-16 flex justify-around items-center px-4 md:hidden z-40 bg-card shadow-[0_-4px_12px_rgba(0,0,0,0.05)] border-t border-border">
        {NAV_ITEMS.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
            ...
            className={({ isActive }) =>
              `p-2 flex flex-col items-center gap-0.5 rounded-xl transition-colors ${
                isActive ? "text-primary bg-background" : "text-slate-400"
              }`
            }
          >
            {({ isActive }) => (
