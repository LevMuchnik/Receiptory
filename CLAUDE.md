...
backend/                 # FastAPI application
  ...
  atomic.py             # Write-temp-then-rename helpers (see Gotchas: shfs).
                        # Also is_contained_name, here so backup/verify.py can
                        # import it without pulling in PyMuPDF, and
                        # STORAGE_MUTATION_LOCK, here so backup/runner.py can
                        # take it without importing storage.py. Two reasons,
                        # not one (see Gotchas)
...
- Design records (per-feature, written before the work): `docs/designs/` — e.g. `docs/designs/mobile-scanner-detection-and-capture.md`, `docs/designs/scanic-1.6-upgrade.md` (the scanic 1.6 upgrade: pinned detection options, the evidence behind them, and the rollback)
...
- **`scanic` is pinned to exactly `1.6.0`, and its detection is pinned too.** `frontend/package.json` carries no `^`, and `SCANIC_DETECTION_OPTIONS` in `frontend/src/lib/opencv-loader.ts` (`lowThreshold: 75`, `highThreshold: 200`, `enableDetectionCascade: false`) plus the per-call `maxDocumentAspectRatio` in `classical-detector.ts` hold 1.6's detector to the edge map 1.0.6 produced. 1.6 came in for its seam-free per-pixel crop; its *default* detector boxed the wood grain under the receipt (11 wrong boxes on 32 labelled frames against 1.0.6's 2). `opencv-loader.test.ts` fails if the options leave the constructor **or if the installed scanic is not 1.6.0** — that tripwire is deliberate, so an `npm update scanic` shows up as a red test rather than a silent detection regression. Re-run the evidence measurement in `docs/designs/scanic-1.6-upgrade.md` before changing either.
...
- **Deleting from any `BACKUP_TREES` directory needs `atomic.STORAGE_MUTATION_LOCK`.** `build_backup` snapshots the database, then `copytree`s those trees while the app keeps running. Its safety argument has two clauses: every snapshot row already had its bytes on disk, **and those bytes are still there when the copy runs**. **The rule: anything that deletes a path under a `BACKUP_TREES` directory takes the lock**, unless that path is under `EXCLUDED_STORAGE_DIRS` (`page_cache`, `tmp`), which the backup never copies. Stated as a rule because the list has been wrong twice — it missed `api/scanner.py`'s `delete_test_frame` (scanner_test_set/), then missed `ingestion/gmail.py`, which unlinks the `storage/converted/<stem>_converted.pdf` that `normalize` writes. That scratch is **not** caught by `_paired_scratch`, which only drops a `<stem>_converted.pdf` that has a `<stem>.pdf` sibling — a temp-file stem never does. Resolve the path, not the variable name. Unguarded, a delete between the snapshot and the walk makes `verify_backup` report damage on a healthy install; a delete *during* the walk defeats `_copy_tolerating_rename`, whose single retry is built for a rename window where the retry finds the settled file (a deleted file never settles), so `copytree` raises `shutil.Error` at the end of the walk and `build_backup` discards the run — one reprocess at 02:00 and there is no backup that night. Both sides take the lock; `runner.py` and `remove_filed` carry reciprocal comments. The lock must never be re-imported or reloaded at runtime: two lock objects exclude nobody, which a test in `test_atomic.py` documents the hard way.
- **`backend/backup/verify.py` is deliberately stdlib-only.** It runs on the restore path, so it must not need PyMuPDF. `storage.py` imports `fitz` at module level, which is why `is_contained_name` lives in `atomic.py` and not next to `get_filed_path`. `test_atomic.py::test_this_module_stays_importable_without_pymupdf` fails if that inverts.
- **`documents.stored_filename` is untrusted input, and LLM-derived.** It is built from `receipt_date` and `vendor_receipt_id`, both verbatim model output, so it changes across reprocesses (measured: the *year* moves, not the receipt id — issue #63). Every consumer goes through `storage.get_filed_path`, which raises `ValueError` rather than joining: an absolute value made `os.path.join` discard the prefix, and `api/export.py` copied whatever it named into the export zip as root. `verify_backup` counts files in `filed/` that no row references as **`filed_orphans`**, reported separately from `problems` — extra bytes are not missing bytes, and grading them as damage would alert nightly on a healthy install.
